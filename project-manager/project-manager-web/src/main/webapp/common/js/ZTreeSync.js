		 ...
		 ZTreeTool.callBack = parameters.callBack ||
		       {
				...
				onRename: ZTreeTool.zTreeOnRename,
		        ...
		        $.ajax({
		            ...
		            success: function(data) {
		               // zNodes = data;
		            	...
		            	var zNodes = dataJson.rtData;
		            	var rtMsg = dataJson.rtMsg;//扩展其它属性
		    			$.fn.zTree.init($("#" + ZTreeTool.zTreeId), setting,zNodes );
		    			if( ZTreeTool.onAsyncSuccessFunc ){
		    				ZTreeTool.onAsyncSuccessFunc(zNodes.length , rtMsg);
...
/*   同步扩展*/
		...
		setting = {
		        ...
		        $.ajax({
		            ...
		            success: function(data) {
		               // zNodes = data;
		            	var dataJson = eval('(' + data + ')');
		            	var zNodes = dataJson.rtData.ztreeData;
		            	var rtMsg = dataJson.rtMsg;//扩展其它属性
		    			$.fn.zTree.init($("#" + ZTreeTool.zTreeId), setting,zNodes );
		    			if( ZTreeTool.onAsyncSuccessFunc ){
   ...
   ,
	...
	comboCtrl:function(target,opts){
		...
		var panel = $("<ul class='ztree zTreeSelectPanel' style='z-index:100000;min-width:230px;'></ul>");
		...
		proxy.setVal=function(val){
			var targetNode = treeDom.getNodeByParam("id",val,null);
			if(targetNode){
				proxy.val(targetNode.name);
				target.val(val);
				treeDom.selectNode(targetNode);
			}
		}
...
/*获取组织机构数，按模块权限和管理范围 对象函数*/
var getPostOrgTreeTool  = {
	/*	@para moduleId:模块Id
		@param inputId : 绑定INputId
		@para param:参数对象   onClickFunc:  点击事件    defaultOpen:默认展开  true-展开   false-不展开*/
		moduleId:'',
		deptIdZTree:'',
		param:null,
		getPostOrgTreeByModule:function (moduleId ,deptIdZTree , param){
			getPostOrgTreeTool.moduleId = moduleId;
			getPostOrgTreeTool.deptIdZTree = deptIdZTree;
			var onClickFunc = null;
			var defaultOpen = 0;
			if(param){
				onClickFunc = param.onClickFunc;
				if(!onClickFunc.defaultOpen){
					defaultOpen = 1;
				}
			}
			var url =  contextPath + "/orgSelectManager/getLeaderPostOrgTree.action?moduleId=" + moduleId + "&defaultOpen="+ defaultOpen;
			var config = {
						zTreeId: deptIdZTree,
						requestURL:url,
			           	onClickFunc:onClickFunc,
						async:false,
						onAsyncSuccess:getPostOrgTreeTool.setOrgPersonSelect
			};
			zTreeObj = ZTreeTool.config(config);
		},
		setOrgPersonSelect:function (dateList,rtMsg){
			ZTreeObj = $.fn.zTree.getZTreeObj(ZTreeTool.zTreeId);
			var nodeName = "";
		    if(ZTreeObj == null){
		    	setTimeout('setOrgPersonSelect()',500);
		    }else{
		    	var node ;
		    }  
		}
