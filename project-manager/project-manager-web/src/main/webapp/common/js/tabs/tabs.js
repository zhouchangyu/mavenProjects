(function($){
	$.addTab=function(c1,c2,opts){
		...
		if(_iframe.length==0){
			_c2.append("<iframe style='height:100%;width:100%;dispaly:none;' frameborder=0></iframe>");
		}
	...
	function renderTab(c1,c2,opts){
		...
		if(!outerTabContainer){
			var left = $("<div style='float:left;display:none' class='tee_tab_pointer_left'><i class=\"glyphicon glyphicon-chevron-left\"></i></div>");
			var right = $("<div style='float:right;display:none' class='tee_tab_pointer_right'><i class=\"glyphicon glyphicon-chevron-right\"></i></div>");
		...
		tab[0].tabContent = c2.find("iframe:first")[0];
