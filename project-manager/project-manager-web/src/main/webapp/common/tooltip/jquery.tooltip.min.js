...
( function($) {
	var helper = {}, current, title, tID, IE = $.browser.msie
			&& /MSIE\s(5\.5|6\.)/.test(navigator.userAgent), track = false;
	$.tooltips = {
		blocked : false,
		defaults : {
			style : "yellow",
		...
		block : function() {
			$.tooltips.blocked = !$.tooltips.blocked
		}
	};
	$.fn
			.extend( {
				tooltips : function(settings) {
					settings = $.extend( {}, $.tooltips.defaults, settings);
					createHelper(settings);
					return this.each( function() {
						$.data(this, "tooltips", settings);
						this.tOpacity = helper.parent.css("opacity");
						this.tooltipsText = this.title;
	...
	function save() {
		if ($.tooltips.blocked || this == current
				|| (!this.tooltipsText && !settings(this).bodyHandler))
			return;
		current = this;
		title = this.tooltipsText;
		...
		if ((!IE || !$.fn.bgiframe) && settings(current).fade) {
			if (helper.parent.is(":animated"))
				...
				helper.parent.is(':visible') ? helper.parent.fadeTo(
						settings(current).fade, current.tOpacity)
	...
	$.extend( {
		showTip : function(content) {
			...
			var myleft = ($("body").width() - tip.width()) / 2
					+ $("body").attr("scrollLeft");
			var mytop = ($("body").height() - tip.height()) / 3
					+ $("body").attr("scrollTop");
