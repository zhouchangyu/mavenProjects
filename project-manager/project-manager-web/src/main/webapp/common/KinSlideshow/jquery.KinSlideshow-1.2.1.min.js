...
$.fn.KinSlideshow = function(settings){
	 ...
	 function KSS_action(){
		...
		if(settings.callback){
			settings.callback($(ksbs).find("#KSS_content").children().eq(0));
		}
	...
	function KSS_move(style){
			var eleIndex = (KSS_changeFlag+1)%KSS_imgaeLength;//元素位置
			//执行回调函数
			if(settings.callback){
				settings.callback($(ksbs).find("#KSS_content").children().eq(eleIndex));
			}
			switch(style){
				case "left":
