...
String tpMode =request.getParameter("tpMode") == null ? "0" : request.getParameter("tpMode");
...
function doInit(){
	...
	console.log(param);
    getDeptTree(param.xfxs,param.blfs)
	//   getAttachId(param.xfxs,"",param.blfs);
...
function dataLoadword(dataList,name , type){
	 ...
	 if(typeof dataList!="undefined"){
		 ...
		 for(var i = 0; i < dataList.length; i++){
			var roleId = dataList[i].docTypeId;
			var name = dataList[i].docType;
		if(roleId != ''){
 ...
 */
function getAttachId(tpMode,isCheckedState,typeName){
	var url = contextPath+"/docAndType/getDataByTpModel.action";
	var para = {tpMode:tpMode,typeName:typeName};
	var jsonRs = tools.requestJsonRs(url,para);
	if(jsonRs.rtState){
		var dataList = jsonRs.rtData;
...
function getDeptTree(tpMode,typeName){
	/* var url = contextPath+"/docAndType/getDataByTpModel.action"; */
	var para = {tpMode:tpMode,typeName:typeName};
	//var param = "";

	 var url =  "<%=contextPath %>/docAndType/getDocAndTypeTree.action";
	var config = {
			...
			param:{tpMode:tpMode,typeName:typeName}
			
		};
	zTreeObj = ZTreeTool.config(config); 	
//	 getAttachId(tpMode,"",typeName);
}
/**
 *  点击
 */
...
function commit(){//getcreateNewWorkData
	...
	if(jsonRs.rtState){
		...
		params["DATA_文号ID"]=autoNumberId;
		params["DOC_ID"]=attachId;//振文附件
		 */
		var params={fType:data.startFlowId};
		var typeName = data.typeName;//类别
		params["DATA_wjlx"]=typeName;//文种类别
		params["DOC_ID"]=data.attachId;//正文附件
		//params["DATA_wjlx"]=param.wenzhong;//文种
		params["DATA_wjz"]=data.wordType;//发文字
		params["DATA_wjnf"]=year;//发文字号
		params["DATA_wjh"]= data.autoNumberId;//编号值
		params["DATA_bt"]=param.biaoti;//标题
		params["runName"]=param.biaoti;//头标题
		params["DATA_ztc"]=param.zhutici; //主题词
		params["DATA_zsdwlist"]=param.divxq;//主送
		doCreate(params,param);
	} 
	return true;
}
function doCreate(params,param){
	var url = contextPath + "/flowRun/createNewWork.action";
	var json = tools.requestJsonRs(url,params);
	if (json.rtState) {
		var insertFlag;
		if(param.blfs_recordid!=null){
			var para={};
			para["recordId"]=param.blfs_recordid;
			para["runId"]=json.rtData.runId;
			var url = contextPath + "/teeRelevantDoc/insertRecordId.action";
			var flag = tools.requestJsonRs(url,para);
			insertFlag = flag.rtState;
		}else{
			insertFlag=true;
		}
		if(insertFlag){
			if(param.xfjbhListArr!=null){
				insertMoreData(json.rtData.runId,param.recordidListArr);//插入相关文件数据
			}
...
//相关文件插入多条数据
function insertMoreData(runId,recordId){
	recordId = recordId.join(",");
	var  insertParams = {runId:runId,recordId:recordId}
	var url = contextPath + "/docAndType/insertMoreData.action";
	var json = tools.requestJsonRs(url,insertParams);
	if(jsonRs.rtState){
